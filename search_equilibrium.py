...
class Simulation:
    ...
    def start(self, case_name, main_input_file, pbed_pos_file, materials, fracs, is_fuel, pbed_universe, division, N_passes, thermal_E=1e-5):
        ...
        self.main_input_file = f'./Inputs/{case_name}/{main_input_file}'
        self.input_files = self.rec_Serpent_file_search(self.main_input_file)
        # Store pebbles positions
        pos = pd.read_csv(f'./Inputs/{case_name}/{pbed_pos_file}', delim_whitespace=True, header=None, names=["x", "y", "z", "r", "uni"])
        # Ensure Outputs folder exists
        os.makedirs('./Outputs', exist_ok=True)

        # Handle name conflicts in Outputs/{case_name}
        output_path = f'./Outputs/{case_name}'
        if os.path.exists(output_path):
            now = datetime.now().strftime("%Y%m%d_%H%M%S")
            output_path = f'./Outputs/{case_name}_{now}'
            print(f"⚠️ Directory already exists. New output directory: {output_path}")
        ...
        self.main_input_file_run = f'./{main_input_file}'
        self.input_files_run = self.rec_Serpent_file_search(self.main_input_file_run)
        
        self.input = CodeInput(self.input_files_run)
...
#%% Input parameters
# Files
case_name = 'gFHR_reference' # Name for folder to target in the Inputs directory, this name will be also uysed to create the directory for the results
main_input_file = 'input' ## path to the input script in your 'case_name_input'
pos_file = 'fpb_pos' # path to the pebble position in your 'case_name_input'
...
materials = {'fuel':'u_fuel_pebble'}#, # material names for each pebble type and corresponding universe (fresh material and universes must be defined in Serpent input)
            #  'fuel_1':'u_fuel1_pebble',
            #  'graphite':'u_graph_pebble'}
fracs = [1] # fraction of pebbles in the pebble bed for each pebble type (might want to convert it to dictionary)
is_fuel = [True] # type of pebble (burnable or not) in the pebble bed for each pebble type (might want to convert it to dictionary)
...
# Simulation
sss_exe = "/global/home/groups/co_nuclear/HxF_tools/serpent2.2.0_HxF_dev_last/sss2" # path to Serpent executable (HxF_tools version)
MPI_mode = False # if using MPI, turn it on
ncores = 32 # number of omp cores per node
...
# Communication
verbosity_mode = 1 # Set if cerberus talks or not
